...
def convert_utc_to_organizer_timezone(
    utc_datetime: datetime, organizer_timezone: str
) -> datetime:
    ...
    try:
        ...
        logging.warning(
            f"Error converting timezone from UTC to {organizer_timezone}: {e}"
        )
        return utc_datetime


def format_datetime_in_organizer_timezone(
    utc_datetime: datetime,
    organizer_timezone: str,
    format_string: str = "%d/%m/%Y à %H:%M",
) -> str:
    """Format datetime string in organizer's timezone"""
    try:
        converted_datetime = convert_utc_to_organizer_timezone(
            utc_datetime, organizer_timezone
        )
        return converted_datetime.strftime(format_string)
    except Exception as e:
        logging.warning(
            f"Error formatting datetime in timezone {organizer_timezone}: {e}"
        )
...
def get_current_time_in_organizer_timezone(organizer_timezone: str) -> datetime:
    ...
    try:
        ...
        logging.warning(
            f"Error getting current time in timezone {organizer_timezone}: {e}"
        )
        return datetime.now()
...
# Optional Redis connection for distributed locks and pub/sub
...
class ConnectionManager:
    ...
    async def _remove_connection(self, websocket: WebSocket, meeting_id: str):
    ...
    async def disconnect(self, websocket: WebSocket, meeting_id: str):
        await self._remove_connection(websocket, meeting_id)

    async def _send_local(self, message: dict, meeting_id: str):
        if meeting_id in self.active_connections:
            dead_connections = []
            ...
            for conn in dead_connections:
                await self._remove_connection(conn, meeting_id)

    async def send_to_meeting(self, message: dict, meeting_id: str):
        await self._send_local(message, meeting_id)
        if redis_client:
            await redis_client.publish(f"ws:{meeting_id}", json.dumps(message))
    ...
    async def redis_listener(self):
        ...
        while True:
            ...
            if message:
                ...
                _, meeting_id = channel.split(":", 1)
                data = json.loads(message["data"])
                await self._send_local(data, meeting_id)
...
class Meeting(BaseModel):
    ...
    title: str
    organizer_name: str
    organizer_timezone: Optional[str] = (
        None  # Fuseau horaire de l'organisateur (ex: "Europe/Paris")
    )
    ...
    report_votes: Dict[str, bool] = Field(
        default_factory=dict
    )  # Votes des scrutateurs {nom: vote}
    ...
    organizer_last_seen: datetime = Field(
        default_factory=datetime.utcnow
    )  # Dernière activité organisateur
    organizer_present: bool = True  # Présence organisateur
    leadership_transferred_to: Optional[str] = (
        None  # Nom du scrutateur ayant reçu le leadership
    )
    auto_deletion_scheduled: Optional[datetime] = (
        None  # Suppression automatique programmée
    )
...
async def create_meeting(meeting_data: MeetingCreate):
    ...
    if not meeting_data.organizer_name or not meeting_data.organizer_name.strip():
        raise HTTPException(
            status_code=400, detail="Le nom de l'organisateur est requis"
        )
    ...
    if len(meeting_data.title.strip()) > 200:
        raise HTTPException(
            status_code=400,
            detail="Le titre de la réunion ne peut pas dépasser 200 caractères",
        )
    if len(meeting_data.organizer_name.strip()) > 100:
        raise HTTPException(
            status_code=400,
            detail="Le nom de l'organisateur ne peut pas dépasser 100 caractères",
        )
...
async def recover_meeting_access(recovery_data: RecoveryRequest):
    ...
    recovery_session = await db.recovery_sessions.find_one(
        {"recovery_code": recovery_code, "expires_at": {"$gt": datetime.utcnow()}}
    )

    if not recovery_session:
        raise HTTPException(
            status_code=404, detail="Lien de récupération invalide ou expiré"
        )
    ...
    return {"meeting": Meeting(**meeting), "message": "Accès récupéré avec succès"}
...
async def organizer_heartbeat(meeting_id: str, heartbeat_data: OrganizerHeartbeat):
    ...
    if (
        meeting["organizer_name"] != heartbeat_data.organizer_name
        and meeting.get("leadership_transferred_to") != heartbeat_data.organizer_name
    ):
...
async def can_close_meeting(meeting_id: str):
    ...
    return {
        ...
        "reason": (
            "Rapport déjà téléchargé"
            if meeting.get("report_downloaded", False)
            else "Le rapport doit être téléchargé avant de fermer la réunion"
        ),
    }
...
async def generate_partial_report(meeting_id: str):
    ...
    if meeting.get("organizer_present", True):
        raise HTTPException(
            status_code=400,
            detail="Rapport partiel disponible seulement quand l'organisateur est absent",
        )
    ...
    try:
        ...
        pdf_path = generate_pdf_report(
            meeting_data, participants, updated_polls, scrutators
        )
        ...
        logger.error(
            f"Error generating partial report for meeting {meeting_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la génération du rapport partiel: {str(e)}",
        )
...
async def get_meeting_by_code(meeting_code: str):
    meeting = await db.meetings.find_one(
        {"meeting_code": meeting_code, "status": "active"}
    )
    if not meeting:
...
async def join_meeting(join_data: ParticipantJoin):
    ...
    if len(join_data.name.strip()) > 100:
        raise HTTPException(
            status_code=400, detail="Le nom ne peut pas dépasser 100 caractères"
        )
    if len(join_data.meeting_code.strip()) != 8:
        raise HTTPException(
            status_code=400, detail="Le code de réunion doit faire 8 caractères"
        )
    ...
    meeting = await db.meetings.find_one(
        {"meeting_code": clean_code, "status": "active"}
    )
    if not meeting:
    ...
    existing = await db.participants.find_one(
        {"name": clean_name, "meeting_id": meeting["id"]}
    )
    if existing:
        raise HTTPException(
            status_code=400, detail="Ce nom est déjà pris dans cette réunion"
        )
    ...
    await manager.send_to_meeting(
        {"type": "participant_joined", "participant": participant.dict()}, meeting["id"]
    )
...
async def add_scrutators(meeting_id: str, scrutator_data: ScrutatorAdd):
    ...
    if not scrutator_data.names or len(scrutator_data.names) == 0:
        raise HTTPException(
            status_code=400, detail="Au moins un nom de scrutateur est requis"
        )
    ...
    for name in scrutator_data.names:
        if not name or not name.strip():
            raise HTTPException(
                status_code=400,
                detail="Les noms de scrutateurs ne peuvent pas être vides",
            )
        if len(name.strip()) > 100:
            raise HTTPException(
                status_code=400,
                detail="Les noms ne peuvent pas dépasser 100 caractères",
            )
    ...
    if len(set(clean_names)) != len(clean_names):
        raise HTTPException(
            status_code=400, detail="Les noms de scrutateurs doivent être uniques"
        )
    ...
    await db.meetings.update_one(
        {"id": meeting_id},
        {"$set": {"scrutators": clean_names, "scrutator_code": scrutator_code}},
    )
...
async def approve_scrutator(scrutator_id: str, approval: ScrutatorApproval):
    ...
    new_status = (
        ScrutatorStatus.APPROVED if approval.approved else ScrutatorStatus.REJECTED
    )
    update_data = {
    ...
    await db.scrutators.update_one({"id": scrutator_id}, {"$set": update_data})
...
async def request_report_generation(
    meeting_id: str, request_data: ReportGenerationRequest
):
    ...
    return {
        ...
        "message": "Génération directe du rapport autorisée",
    }
...
async def submit_scrutator_vote(meeting_id: str, vote_data: ScrutatorReportVote):
    ...
    if not meeting.get("report_generation_pending", False):
        raise HTTPException(
            status_code=400, detail="Aucune demande de génération en cours"
        )
...
async def join_as_scrutator(join_data: ScrutatorJoin):
    ...
    meeting = await db.meetings.find_one(
        {"scrutator_code": clean_code, "status": "active"}
    )
    if not meeting:
        raise HTTPException(
            status_code=404, detail="Code de scrutateur invalide ou réunion inactive"
        )
    ...
    if clean_name not in meeting.get("scrutators", []):
        raise HTTPException(
            status_code=403,
            detail="Nom non autorisé pour cette réunion en tant que scrutateur",
        )
...
async def approve_participant(participant_id: str, approval: ParticipantApproval):
    ...
    new_status = (
        ParticipantStatus.APPROVED if approval.approved else ParticipantStatus.REJECTED
    )
    await db.participants.update_one(
        {"id": participant_id}, {"$set": {"approval_status": new_status}}
    )
...
async def create_poll(meeting_id: str, poll_data: PollCreate):
    ...
    if not poll_data.question or not poll_data.question.strip():
        raise HTTPException(
            status_code=400, detail="La question du sondage est requise"
        )
    ...
    for i, option in enumerate(poll_data.options):
        if not option or not option.strip():
            raise HTTPException(
                status_code=400, detail=f"L'option {i+1} ne peut pas être vide"
            )
        if len(option.strip()) > 200:
            raise HTTPException(
                status_code=400,
                detail=f"L'option {i+1} ne peut pas dépasser 200 caractères",
            )
...
async def start_poll(poll_id: str):
    ...
    # Notify participants
    await manager.send_to_meeting(
        {"type": "poll_started", "poll_id": poll_id}, poll["meeting_id"]
    )
...
async def close_poll(poll_id: str):
    ...
    # Notify participants
    await manager.send_to_meeting(
        {"type": "poll_closed", "poll_id": poll_id}, poll["meeting_id"]
    )
...
async def submit_vote(vote_data: VoteCreate):
    ...
    async def record_vote(session):
        vote = Vote(poll_id=vote_data.poll_id, option_id=vote_data.option_id).dict()
        result = await db.polls.update_one(
            {"id": vote_data.poll_id, "options.id": vote_data.option_id},
            {"$inc": {"options.$.votes": 1}},
            session=session,
        )
        if result.matched_count == 0:
            raise HTTPException(status_code=400, detail="Option invalide")
        await db.votes.insert_one(vote, session=session)

    session = await client.start_session()
    if redis_client:
        lock = redis_client.lock(f"poll:{vote_data.poll_id}", timeout=5)
        async with lock:
            async with session.start_transaction():
                await record_vote(session)
    else:
        async with session.start_transaction():
            await record_vote(session)
...
def generate_pdf_report(
    meeting_data, participants_data, polls_data, scrutators_data=None
):
    ...
    story.append(
        Paragraph(f"<b>Réunion:</b> {meeting_data['title']}", styles["Normal"])
    )
    story.append(
        Paragraph(
            f"<b>Organisateur:</b> {meeting_data['organizer_name']}", styles["Normal"]
        )
    )
    story.append(
        Paragraph(
            f"<b>Code de réunion:</b> {meeting_data['meeting_code']}", styles["Normal"]
        )
    )
    ...
    if meeting_data.get("created_at"):
        if isinstance(meeting_data["created_at"], str):
            created_at = datetime.fromisoformat(
                meeting_data["created_at"].replace("Z", "+00:00")
            )
        else:
            created_at = meeting_data["created_at"]
        formatted_created_at = format_datetime_in_organizer_timezone(
            created_at, organizer_timezone
        )
        story.append(
            Paragraph(
                f"<b>Date de création:</b> {formatted_created_at}", styles["Normal"]
            )
        )
    ...
    story.append(
        Paragraph(
            f"<b>Date de génération:</b> {current_time.strftime('%d/%m/%Y à %H:%M')}",
            styles["Normal"],
        )
    )
    ...
    if scrutators_data and len(scrutators_data) > 0:
        ...
        for i, scrutator in enumerate(scrutators_data, 1):
            ...
            if isinstance(scrutator["added_at"], str):
                added_datetime = datetime.fromisoformat(
                    scrutator["added_at"].replace("Z", "+00:00")
                )
            else:
                added_datetime = scrutator["added_at"]

            added_time = format_datetime_in_organizer_timezone(
                added_datetime, organizer_timezone
            )
            scrutators_table_data.append([str(i), scrutator["name"], added_time])

        scrutators_table = Table(
            scrutators_table_data, colWidths=[0.5 * inch, 3 * inch, 1.5 * inch]
        )
        scrutators_table.setStyle(
            TableStyle(
                [
        ...
        story.append(
            Paragraph(
                f"<b>Total des scrutateurs:</b> {len(scrutators_data)}",
                styles["Normal"],
            )
        )
        story.append(Spacer(1, 30))
    ...
    approved_participants = [
        p for p in participants_data if p["approval_status"] == "approved"
    ]
    ...
    if approved_participants:
        ...
        for i, participant in enumerate(approved_participants, 1):
            ...
            joined_time = format_datetime_in_organizer_timezone(
                joined_datetime, organizer_timezone, "%H:%M"
            )
            participants_table_data.append([str(i), participant["name"], joined_time])

        participants_table = Table(
            participants_table_data, colWidths=[0.5 * inch, 3 * inch, 1.5 * inch]
        )
        ...
        story.append(
            Paragraph(
                f"<b>Total des participants approuvés:</b> {len(approved_participants)}",
                styles["Normal"],
            )
        )
    else:
    ...
    if polls_data:
        for i, poll in enumerate(polls_data, 1):
            # Poll question
            story.append(
                Paragraph(f"<b>Sondage {i}:</b> {poll['question']}", styles["Heading3"])
            )
            ...
            if total_votes > 0:
                ...
                for option in poll["options"]:
                    percentage = (
                        (option["votes"] / total_votes * 100) if total_votes > 0 else 0
                    )
                    results_data.append(
                        [option["text"], str(option["votes"]), f"{percentage:.1f}%"]
                    )
                ...
                results_table = Table(
                    results_data, colWidths=[3 * inch, 1 * inch, 1 * inch]
                )
                ...
                story.append(
                    Paragraph("Aucun vote enregistré pour ce sondage", styles["Normal"])
                )
        ...
        story.append(
            Paragraph(
                "Aucun sondage n'a été créé lors de cette réunion", styles["Normal"]
            )
        )
    ...
    story.append(
        Paragraph(
            "Toutes les données de cette réunion ont été supprimées après génération de ce rapport",
            footer_style,
        )
    )
...
async def generate_meeting_report(meeting_id: str):
    ...
    # Polls already contain up-to-date results
    updated_polls = polls
    ...
    try:
        ...
        if poll_ids:
            delete_votes_result = await db.votes.delete_many(
                {"poll_id": {"$in": poll_ids}}
            )
            logger.info(
                f"Deleted {delete_votes_result.deleted_count} votes for meeting {meeting_id}"
            )

        # Delete polls
        delete_polls_result = await db.polls.delete_many({"meeting_id": meeting_id})
        logger.info(
            f"Deleted {delete_polls_result.deleted_count} polls for meeting {meeting_id}"
        )

        # Delete participants
        delete_participants_result = await db.participants.delete_many(
            {"meeting_id": meeting_id}
        )
        logger.info(
            f"Deleted {delete_participants_result.deleted_count} participants for meeting {meeting_id}"
        )

        # Delete scrutators
        delete_scrutators_result = await db.scrutators.delete_many(
            {"meeting_id": meeting_id}
        )
        logger.info(
            f"Deleted {delete_scrutators_result.deleted_count} scrutators for meeting {meeting_id}"
        )

        # Delete scrutator access records (if any)
        try:
            delete_access_result = await db.scrutator_access.delete_many(
                {"meeting_id": meeting_id}
            )
            logger.info(
                f"Deleted {delete_access_result.deleted_count} scrutator access records for meeting {meeting_id}"
            )
        except:
    ...
    while True:
        try:
            ...
            for meeting in active_meetings:
                meeting_id = meeting["id"]
                organizer_last_seen = meeting.get(
                    "organizer_last_seen", meeting["created_at"]
                )
                ...
                if (
                    time_elapsed.total_seconds() > 300 and organizer_present
                ):  # 5 minutes
                    # Marquer l'organisateur comme absent
                    await db.meetings.update_one(
                        {"id": meeting_id}, {"$set": {"organizer_present": False}}
                    )

                    # Vérifier s'il y a des scrutateurs approuvés
                    approved_scrutators = (
                        await db.scrutators.find(
                            {"meeting_id": meeting_id, "approval_status": "approved"}
                        )
                        .sort("approved_at", 1)
                        .to_list(100)
                    ...
                    if approved_scrutators:
                        ...
                        await db.meetings.update_one(
                            ...
                            {
                                "$set": {
                                    "leadership_transferred_to": senior_scrutator[
                                        "name"
                                    ]
                                }
                            },
                        ...
                        logger.info(
                            f"Leadership transferred to {senior_scrutator['name']} for meeting {meeting_id}"
                        )
                    else:
                ...
                if auto_deletion and datetime.utcnow() >= auto_deletion:
                    ...
                    active_connections = len(
                        manager.active_connections.get(meeting_id, [])
                    )

                    if active_connections == 0:
                        # Pas de connexions actives - supprimer la réunion
                        await cleanup_meeting_data(
                            meeting_id, "auto_deletion_time_limit"
                        )
                        logger.info(
                            f"Auto-deleted meeting {meeting_id} due to time limit and no active connections"
                        )
                    else:
        ...
        await db.votes.delete_many(
            {
                "poll_id": {
                    "$in": [
                        poll["id"]
                        for poll in await db.polls.find(
                            {"meeting_id": meeting_id}
                        ).to_list(1000)
                    ]
                }
